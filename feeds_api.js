...
function display_listings(items) {
  ...
  listing_entry.innerHTML += '<div class="listing-link"><a href="'+item['link']+'"><button><span>&#128489;</span>Contact Informant</button></a></div>';
...
function fetch_listings() {
  ...
  var search_terms = '#pricestr';
  var search_category = $('#search_category').val();
  var valid_categories = ['product','service','job','service'];
  ...
  fetch(url).then((res) => {
    res.text().then((res_text) => {
      ...
      json_text['serp'].forEach((item) => {
        ...
        var listing_details = {'guid': guid_bech32, 'title': title, 'description': description, 'timestamp': timestamp, 'link': link, 'category': category, 'image': img_link};
